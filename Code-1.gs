...
// Sends a personalized email to many recipients at once (Email Center).
// Each recipient gets their own individual email (not a group Bcc) so the
// {name} placeholder can be personalized and addresses stay private.
// Google's daily quota applies (~100/day on a plain Gmail account,
// much higher on Google Workspace) — the loop stops early if it's hit.
function apiSendBulkEmail(p) {
  var auth = requireRelayKey(p); if (!auth.ok) return auth.response;
  var missing = validateRequired(p, ['recipients', 'subject', 'message']);
  if (missing.length) return jsonResponse(false, 'Missing fields: ' + missing.join(', '));

  var recipients = p.recipients;
  if (typeof recipients === 'string') recipients = JSON.parse(recipients);
  if (!recipients || recipients.length === 0) return jsonResponse(false, 'No recipients provided.');

  var sent = 0, failed = 0, quotaHit = false;
  for (var i = 0; i < recipients.length; i++) {
    var r = recipients[i];
    if (!r.email) { failed++; continue; }
    try {
      var remaining = MailApp.getRemainingDailyQuota();
      if (remaining <= 0) { quotaHit = true; break; }
      var body = String(p.message).replace(/\{name\}/g, r.name || 'Student');
      MailApp.sendEmail({ to: r.email, subject: p.subject, body: body });
      sent++;
    } catch (err) {
      failed++;
    }
  ...
  var msg = 'Sent to ' + sent + ' of ' + recipients.length + ' recipient(s).';
  if (failed > 0) msg += ' ' + failed + ' failed.';
  if (quotaHit) msg += ' Daily email quota reached — remaining recipients were not sent.';
  return jsonResponse(true, msg, { sent: sent, failed: failed, quotaHit: quotaHit });
}
